...
async function generateSecureToken(email: string, expiryTime: number = 15 * 60 * 1000): Promise<string> {
  ...
  const secret = isDevelopment ? ((typeof import.meta !== 'undefined' && import.meta.env.VITE_JWT_SECRET) || 'dev-secret-key') : 'CLIENT_SIDE_PLACEHOLDER';
...
const useSupabase = typeof import.meta !== 'undefined' && (import.meta.env.VITE_USE_SUPABASE === 'true' || import.meta.env.USE_SUPABASE === 'true');
