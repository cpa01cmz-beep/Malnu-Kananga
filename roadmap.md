...
## Version History
    ...
    |---------|------|---------|
    | 3.3.4 | 2026-01-31 | Fix Two P1 Bugs: Hardcoded localStorage Keys & Math.random() Mock Data (Issues #1269, #1270): Issue #1269 - Fixed remaining hardcoded localStorage keys in emailNotificationService.ts (5 keys replaced with STORAGE_KEYS constants); Issue #1270 - Removed all Math.random() calls from StudyPlanAnalytics.tsx calculateAnalytics function; Replaced mock data with calculated/default values (0, null, empty arrays); Fixed test mocking in StudyPlanAnalytics.test.tsx (authService.getCurrentUser → apiService.authAPI.getCurrentUser); TypeScript type checking passed; ESLint linting passed; Updated blueprint.md, roadmap.md, task.md |
