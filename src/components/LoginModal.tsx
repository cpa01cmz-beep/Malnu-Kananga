...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  return (
    <div
      ...
      role="dialog"
    >
      <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-float w-full max-w-md m-4 transform transition-all duration-300 scale-95 opacity-0 animate-scale-in flex flex-col max-h-[90vh] border border-neutral-200 dark:border-neutral-700">
        <div className="flex justify-between items-center px-6 py-5 border-b border-neutral-200 dark:border-neutral-700">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-lg text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-all focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-800"
         ...
         <div className="p-6 overflow-y-auto">
            <div className="mb-6 p-5 bg-neutral-50 dark:bg-neutral-900/50 rounded-xl border border-neutral-200 dark:border-neutral-700">
                ...
                <div className="grid grid-cols-3 gap-2.5">
                    <button onClick={() => handleSimulatedLogin('student')} className="px-3 py-3 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-600 rounded-lg text-xs sm:text-sm font-medium hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1">
                        Siswa
                    </button>
                    <button onClick={() => handleSimulatedLogin('teacher')} className="px-3 py-3 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-600 rounded-lg text-xs sm:text-sm font-medium hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1">
                        Guru
                    </button>
                    <button onClick={() => handleSimulatedLogin('admin')} className="px-3 py-3 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs sm:text-sm font-semibold hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1">
                        Admin
                    </button>
                </div>
          ...
          {formState === 'success' ? (
            <div className="text-center py-6">
              ...
              <button
                onClick={onClose}
                className="mt-6 w-full flex justify-center py-3 px-5 border border-transparent rounded-lg text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all duration-200 shadow-md hover:shadow-lg"
              >
                Selesai
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-white dark:bg-neutral-700 text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 font-medium placeholder-neutral-400 dark:placeholder-neutral-500" placeholder="anda@email.com" />
              </div>
              <div>
                <label htmlFor="password" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">Password</label>
                <input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-white dark:bg-neutral-700 text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 font-medium placeholder-neutral-400 dark:placeholder-neutral-500" placeholder="Masukkan password" />
              </div>
